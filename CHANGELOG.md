...
- **General**:
  ...
  - No longer requires _mady_ as peer dependency.
- **Server**:
...
- **Client**:
  - _bugfix_: General: show log-in form when logged out.
  - Files: highlight those that don't seem to be in use.
  - _bugfix_: Previewer: fix full-screen link.
...
- **Client**:
  ...
  - _bugfix_: Item list: fix sort by date field.
...
- **Client**:
  ...
  - After _Restart server_, refresh in 10 s (rather than 3).
...
- **General**:
  - Add _Restart server_ developer action
- **Client**:
...
- **Client**:
  ...
  - Add _Pull from repo_, _Push to repo_ actions.
